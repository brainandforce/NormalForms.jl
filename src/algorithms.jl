...
function zero_row_and_col!(
    ...
    iterations = 0
    # The cycle: Zero the row, reduce the column, zero the column, reduce the row
    while !is_row_zero_after(A, k) || !is_col_zero_after(A, k)
        iterations += 1
        zero_row!(A, V, k, ki)
        ...
        # TODO: what's a resonable bound on the number of iterations before we should error?
        iterations > length(A) && error("Unreasonable amount of iterations (file an issue)")
    end
end
